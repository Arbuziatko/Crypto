__author__ = 'Sabina'


def xor_strings(xs,ys):
    """Xoring two hex strings, returns hex string"""
    return "".join(chr(ord(x) ^ ord(y)) for x, y in zip(xs.decode("hex"), ys.decode("hex"))).encode("hex")


def space_possible(xc,yc,c):
    """ First xoring xc xor 20 to get a key, then yc xor key to check if the letter is lowercase of c
    """
    possible_key = xor_strings(xc, "20")
    possible_letter = xor_strings(yc, possible_key)
    if int(c, 16) + 32 == int(possible_letter, 16):
        return True


def check_spaces(xs, ys, key_list):
    """Checks for possible spaces found by xoring xs and ys strings together,
    if found, generates both possible keys on corresponding positions,
    returns key list which is a list of possible lists"""
    xored_str = xor_strings(xs, ys)
    for j in range(0, len(xored_str), 2):
            if int(xored_str[j:j+2], 16) < 91 and int(xored_str[j:j+2], 16) > 64:
                if space_possible(xs[j:j+2],ys[j:j+2],xored_str[j:j+2]):
                    key_list[(j/2)] = [xor_strings(xs[j:j+2], "20"),xor_strings(ys[j:j+2], "20")]

    return key_list


def key_generator(str,sl,index):
    """Quick generator for trying the specific key,
    str is a text we want to get,
    s1 is a list of texts, index is index of a text we want to xor"""
    return xor_strings(sl[index],str.encode("hex"))


def string_to_hex_list(xs):
    """Converts string to a hex list"""
    xs_list = list()
    for j in range(0, len(xs), 2):
        xs_list.append(xs[j:j+2])

    return xs_list


def possible_letter(xs_list,key,index):
    """Returns key xor hex on a specific position in the xs_list"""
    return xor_strings(xs_list[index],key)


def decypher(sl, key):
    """Decyphering texts is sl with specific key"""
    for str in sl:
        temp = ""
        for char in xor_strings(str,key).decode("hex"):
            temp += chr(ord(char))
        print temp


def hex_to_character(xc):
    """Converts hex to ascii character"""
    return "".join(chr(ord(char)) for char in xc.decode("hex"))


def decrypt_messages(msgs, key):
    """Decrypts with a key"""
    for msg in msgs:
        msg_list = string_to_hex_list(msg)
        i = 0
        for k in key:
            if k != "00":
                msg_list[i] = hex_to_character(xor_strings(msg_list[i], k))
            else:
                msg_list[i] = "-"
            i += 1
        print "".join(msg_list)

if __name__ == "__main__":
    l = ["",
        "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f",
        "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb",
        "",
        "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070",
        "",
        "",
        "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3",
        "",
        "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83",
        "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"]
    target = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"

    # Used to check for spaces
    key_prob = ["00" for x in range(len(target)/2)]
    key_prob = check_spaces(l[0],target,key_prob)


    # HERE COMES THE MASTER KEY FOR TARGET MESSAGE
    master_key = ['66', '39', '6e', '89', 'c9', 'db', 'd8', 'cc', '98', '74',
                  '35', '2a', 'cd', '63', '95', '10', '2e', 'af', 'ce', '78',
                  'aa', '7f', 'ed', '28', 'a0', '7f', '6b', 'c9', '8d', '29',
                  'c5', '0b', '69', 'b0', '33', '9a', '19', 'f8', 'aa', '40',
                  '1a', '9c', '6d', '70', '8f', '80', 'c0', '66', 'c7', '63',
                  'fe', 'f0', '12', '31', '48', 'cd', 'd8', 'e8', '02', 'd0',
                  '5b', 'a9', '87', '77', '33', '5d', 'ae', 'fc', 'ec', 'd5',
                  '9c', '43', '3a', '6b', '26', '8b', '60', 'bf', '4e', 'f0',
                  '3c', '9a', '61']


    decrypt_messages(l, master_key)